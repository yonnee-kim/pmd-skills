...
L3 계약(prior)과 구현 코드 간 drift를 감지하고 어느 방향으로 맞출지 제안한다. 매핑 규약은 `shared/mapping-spec.md` — 먼저 읽을 것.

## 실행

1. 매핑 규약(`shared/mapping-spec.md`) Read
...
| 관찰                                  | 방향          | 제안                            |
| ------------------------------------- | ------------- | ------------------------------- |
| 코드에 있음, 문서에 없음              | `code-ahead`  | L3 문서 업데이트                |
| 문서에 있음, 코드에 없음              | `doc-ahead`   | 미구현 계약 리스트로 남김       |
| 양쪽 있으나 시그니처/필드가 다름      | `divergent`   | 사용자에게 기준 선택 요청       |
| `implements` 경로 stale (코드 없음)   | `path-stale`  | L3 경로 수정 또는 `implements` 제거 |
...
- `implements` 누락된 L3는 인덱스에 없음 → 미탐지. 이 경우 prior 엔진(`pyramid`)에서 메타데이터 보강 요청
- glob·심볼 단위 매핑은 현 단계에서 미지원 (규약 참조)
- 변경이 많을 때 파일 하나씩 처리 — 한 번에 모든 drift를 일괄 수정하려 하지 말 것
